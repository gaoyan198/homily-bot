...
# PART 0 — THE THREE NUMBERS YOU MUST SET

## 1 · Monthly amount — what you can actually afford, every month

This is **income you can commit each month**, not an average to front-load
...
Separate from income. This is the only thing that lets you buy heavier in the
trough window, and **if it is zero, you simply contribute your monthly amount
flat and that is a perfectly good plan.**

> **⚠️ Correction, 2026-08-21.** An earlier version of this file told you to
> contribute "2.5× base rate" (e.g. **S$16,945/mo** at a S$4,250 base) during
> the window. **That was wrong and unaffordable.** It silently assumed a large
> reserve existed. The weighting is not extra income you must find — it is
> **when you deploy cash you already have.** See §49.2.

## 3 · W — the size cap on the leveraged engine (only matters once 🐂 BULL)

**W is the most money allowed inside the leveraged account, and therefore the
...
# HOW THE RESERVE GETS DEPLOYED

**Rule: split your reserve across the four months of the trough window
(25 Aug → 23 Dec). Send `monthly amount + reserve ÷ 4` in each of those
months. After the window, back to the monthly amount.**

Measured against the alternatives, with a S$50,000 reserve:

| when you deploy the reserve | c2 analog | c3 analog |
|---|---|---|
| **across the trough window** | **22.81 BTC** | **5.56 BTC** |
...
Window-timing beat spreading by **+10.4% / +12.4%**. That is the entire
weighting edge, and it costs you nothing you do not already have.
...
| | weight | why |
|---|---|---|
| inside the trough window (`🎯` shows) | **monthly + reserve ÷ 4** | the cycle says the bottom is here |
| outside it | **monthly amount** | plain, affordable, unchanged |
