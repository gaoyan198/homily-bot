...
| 101 | **Daily candle colour in the ledger** (owner-noticed 2026-07-13 via Danny's MU yellow-candle post) — `daily_candle()` already computes RED/YELLOW/NEUTRAL, the digest renders it (`dY` + `dip d{n}` pullback clock, #78) and the dashboard colours the card (#83), but it's the one engine output NOT persisted to `homily_signals_log.csv` (the log's `wk_circle` is the *weekly* circle, not the daily candle) — so a "did we flag MU yellow on 2026-07-10?" audit needs a live recompute, and the per-name daily-candle signal can't be replayed from the committed log. Add a `candle` column (forward rows accrue only from ship, same time-sensitive pattern as #80's `whale_rank` / #89's `rs6_rank`). Pure measurement | S | none (pure ledger append — the column gates nothing); unlocks a later daily-candle event study, 2021 control included |
