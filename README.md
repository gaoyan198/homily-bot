...
| `homily_deliver.py` | Alternate delivery (#118a): `HOMILY_DELIVERY=file:<dir>` routes the digest, chart cards and documents to one self-contained HTML bundle per day instead of Telegram, behind the same three `send*` functions; dormant when unset. Drilled 2026-09-11 from a launchd-scheduled run (#118b) |
| `homily_budget.py` + `module_budget.json` | Complexity budget (#116): live modules derived from the import graph, everything else registered as tool / gate / pending (dated read) / null (`archive_by`); validate [85] fails on an unclassified module, a rotten read, an overdue null, or a count above the cap. `python3 homily_budget.py` prints the census |
| `COLD_START.md` | The succession runbook (#115): fresh clone → green validate → local digest → buy-day rehearsal → sleeve validate → vault restore drill, the full secrets/variables inventory, the restore paths, and the July drill protocol + log. **Operators with no context start here** |
| `homily_verdict.py` + `verdict_freeze.json` | R-2029/R-2036 verdict freeze (#117): the pre-registered three-fork rule (ROADMAP §3/§4) and the #71 band method (`homily_bootstrap` source) pinned by hash; validate [82] fails on drift; `--repin` + a same-day PRD §8.5 note is the only sanctioned change |
| `homily_vault.py` + `vault/` | Bars vault (#113): one frozen full-history base per year + a self-contained monthly delta of every symbol the run touches, committed by CI; `HOMILY_BARS_SOURCE=vault` makes every fetch read the vault instead of Yahoo (raw bars bit-exact), `--drill` proves a digest + a backtest reproduce with the network off, `--check` verifies integrity. The digest's data-QA footer prints one line the morning a delta catches Yahoo rewriting base history |
...
| `homily_clusters.py` | Concentration lens (#29): 90d-correlation clusters of the book, one digest line + a ⭐-deepens-cluster nudge; info-only. #97 adds the cross-book view: swing sleeve + external ESPP folded in (correlation math untouched), a `🔗 across both books` line + G5 warning when they deepen a cluster; the gambit order sheet warns when the two books share >2 names |
...
| `homily_ops.py` | Ops-readiness (#99): one standing `⏳ SETUP` line listing the owner's unset switches (MARGIN_ZERO / IBKR Flex secrets / BUY_BUDGET_USD) + margin-paydown progress; silent when the board is clean. Pairs with the one-shot KILL-A proximity warning (`gambit_live.kill_watch`, equity first below 80% of contributed) |
...
| `homily_swing.py` | SWING (paper) digest block (#90): P2-gate counters from the merged sleeve's committed snapshot — read-only, fetch-free, never a trade signal. Monthly report carries the #95 flywheel line (cumulative banked skims + sleeve score); the skim itself is mechanized in `gambit/gambit_live.py` (`maybe_skim`) and routed into the 🛒 BUY DAY block — quarter-end profit funds the DCA, kill-safe (skims never touch `contributed`/`realized`). The monthly report also carries the #96 A5 A/B stop-cost section (`gambit/gambit_ab.py`, read-only over both journals): what the mandated stops cost or saved vs the no-stops paper leg, verdict gated at 26 live weeks / 20 closed trades, report-only. And the #100 cost-reconcile section (`gambit/gambit_reconcile.py`): actual IBKR financing/fills vs the modeled 5.8% + Monday opens — the true financing rate feeds LEVERAGE.md §5, and slippage past the 0.35% stress arm prints red |
