...
| `F:n/3` | EDGAR checks, latest fiscal year vs prior: revenue +10%+ · net income > 0 OR operating cash flow > 0 · share dilution <12%/yr | feeds exactly one rule: PLAYBOOK §5.2 (8+ weeks in ⚪ **and** F:0–1 → sell half; 12→8 with the #51 promotion, 2026-07-22) — the "broken business, not just broken chart" test | info-only as a tag; the §5.2 rule it feeds is the system's best-measured arm (§6) |
...
| `wk RED…` / `mUP…` (the `…`) | that trend engine's deciding bar is still forming — `m…` inside the month's first 10 sessions (per the name's own calendar), `w…` on a Mon–Thu print | Danny's "to be finalized": measured over 5y/both universes (`homily_provisional_backtest.py`, 54k day-obs), a mid-period `monthly_up` later disagreed with its settled month on **9.9%** of days (⅔ of that in the first 10 sessions), the weekly circle on 4.2%, and **7.5%** of days printed a state class the completed bar would contradict — so an early-month state flip deserves less weight than a settled one | display-only (#106); the signal math still uses every bar (R1) — the mark changes presentation, nothing downstream |
